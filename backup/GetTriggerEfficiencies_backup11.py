...
def getSplit(l, n):
    ...
    print("stepsize")
    print(stepsize)
    for i in range(stepsize, len(l)-stepsize, stepsize):
       # print(i)
       # print(i-stepsize)
        m.append(l[i-stepsize:i])
...
for i in channel:
    for w in whichType:
        for z in whichLepton:
            for g in whichleptonFeature:
                print(i+"_Channel_"+w+"_"+z+"_"+g+"_Order")
#                i+"_Channel_"+w+"_"+z+"_"+g+"_Order" = []
################x
################ Here we are generating ordered arrays containing the indices of array XY 
################ No need to write a huge amount of functions oneself. It's the machine's job to generate the program code and to execute it --> more reliable against human errors.
################
for i in channel:
    for w in whichType:
        for z in whichLepton:
            for g in whichleptonFeature:
                if g == "pt": index_whichleptonFeature = "0"
                elif g == "eta": index_whichleptonFeature = "1"
                elif g == "iso": index_whichleptonFeature = "2"
                if not index_whichleptonFeature: sys.exit("Something went wrong in assigning index_whichleptonFeature a value. Please check.")
                print(str(i)+"_Channel_"+w+"_"+z+"_"+g+"_Order = [indices_ordered[0] for indices_ordered in sorted(enumerate("+i+"_Channel_"+w+"_"+z+"_pt_eta_iso), key=lambda elem: elem[1]["+index_whichleptonFeature+"])]")
                exec(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order = [indices_ordered[0] for indices_ordered in sorted(enumerate("+str(i)+"_Channel_"+str(w)+"_"+str(z)+"_pt_eta_iso), key=lambda elem: elem[1]["+str(index_whichleptonFeature)+"])]")
                print(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order"))
                numberOfTotalBins = 6.

#                numberOfMaxElementsPerBin = int(round(len(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order"))/numberOfTotalBins))
 #               print("numberOfMaxElementsPerBin")
  #              print(numberOfMaxElementsPerBin)
                print(len(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order")))
   #             print(len(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order"))/numberOfTotalBins)
    #            print(round(len(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order"))/numberOfTotalBins))
                
     #           exec(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order = splitListintoSubListsOfPieceSize("+str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order, numberOfMaxElementsPerBin)")
                exec(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order = getSplit("+str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order, numberOfTotalBins)")
                print(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order")
                for bin in eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order"):
                     print(bin)
                     # Start evaluating bin value:
                     BinValueDLTrue = 0.
                     SF_BinValueDLTrue = 0.
                     BinValueDLFalse = 0.
                     SF_BinValueDLFalse = 0.
                     BinValueSLTrue = 0.
                     SF_BinValueSLTrue = 0.
                     
                     for item_bin in bin:
                          print(item_bin)
                          print(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_pt_eta_iso["+str(item_bin)+"]"))x

#                print("print("+i+"_xsChannel_"+w+"_"+z+"_"+g+"_Order)")
                for t in inWhichSubsetsToDivide:
                    if "Njets" in t[0]:
                        index_inWhichSubsetsToDivide = "0"
                    elif "Nvertices" in t[0]:
                        index_inWhichSubsetsToDivide = "1"
                    elif "MET" in t[0]:
                        index_inWhichSubsetsToDivide = "2"
                    exec(i+"_Channel_"+w+"_"+z+"_"+g+"_Order_"+t[0]+" = [indices_ordered[0] for indices_ordered in sorted(enumerate("+i+"_Channel_"+w+"_"+z+"_pt_eta_iso), key=lambda elem: elem[1]["+index_whichleptonFeature+"]) if "+i+"_Channel_"+w+"_njet_nvertex_met[indices_ordered[0]]["+index_inWhichSubsetsToDivide+"] "+t[1]+"] ")
                 #   c = compile(i+"_Channel_"+w+"_"+z+"_"+g+"_Order_"+t[0]+"[indices_ordered[0] for indices_ordered in sorted(enumerate("+i+"_Channel_"+w+"_"+z+"_pt_eta_iso), key=lambda elem: elem[1]["+index_whichleptonFeature+"]) if "+i+"_Channel_"+w+"_njet_nvertex_met[indices_ordered[0]]["+index_inWhichSubsetsToDivide+"] "+t[1]+"] ")
                  #  eval(c)
                    
                    print(i+"_Channel_"+w+"_"+z+"_"+g+"_Order_"+t[0]+" = [indices_ordered[0] for indices_ordered in sorted(enumerate("+i+"_Channel_"+w+"_"+z+"_pt_eta_iso), key=lambda elem: elem[1]["+index_whichleptonFeature+"]) if "+i+"_Channel_"+w+"_njet_nvertex_met[indices_ordered[0]]["+index_inWhichSubsetsToDivide+"] "+t[1]+"] ")

                    if int(round(len(eval(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order_"+str(t[0])))/float(numberOfTotalBins))) != 0:

                         exec(str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order_"+str(t[0])+" = getSplit("+str(i)+"_Channel_"+str(w)+"_"+str(z)+"_"+str(g)+"_Order_"+str(t[0])+", numberOfTotalBins)")
                    

print("ee_Channel_data_2nd_Lepton_eta_Order_3AndMoreNjets")
print(ee_Channel_data_2nd_Lepton_eta_Order_3AndMoreNjets)
print("ee_Channel_data_2nd_Lepton_pt_eta_iso")
print(ee_Channel_data_2nd_Lepton_pt_eta_iso)
print("----------------------------------------------")
...
for i in ee_Channel_data_2nd_Lepton_eta_Order_3AndMoreNjets:
     print(i)
     for ii in i:
          print(ee_Channel_data_2nd_Lepton_pt_eta_iso[ii])
          print("<<<<<<")
          print(ee_Channel_data_njet_nvertex_met[ii])
          print("\n ---------------")

# Divide arrays into bins:
